...
Actuation::Actuation(uint8_t addr1, uint8_t addr2, TwoWire &wire) : _gpioExpanderOutput(addr1, wire), _gpioExpanderInput(addr2, wire) {}

void Actuation::init() {
  uint8_t configOutputs = PCA6408A_ALL_INPUT;  // init in high impedance
...
void Actuation::setDigital(uint8_t port, uint8_t output) { _gpioExpanderOutput.setDigital(port, output); }
